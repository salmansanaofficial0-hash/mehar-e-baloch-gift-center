      ...
      <div className="section-shell flex items-center justify-between py-3 sm:py-4">
        <Link to="/" className="flex min-w-0 items-center gap-2 sm:gap-3">
          <img
            src={brand.logo}
            alt={brand.name}
            className="h-10 w-10 shrink-0 rounded-full object-cover object-center sm:h-11 sm:w-11"
          />
          <div className="min-w-0">
            <p className="truncate font-display text-base font-bold text-burgundy sm:text-xl">Mehr-e-Baloch</p>
            <p className="truncate text-[9px] uppercase tracking-[0.2em] text-slate-500 sm:text-[10px] sm:tracking-[0.25em]">Cosmetics</p>
          </div>
        </Link>
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `text-sm font-medium transition ${isActive ? 'text-burgundy' : 'text-slate-600 hover:text-rust'}`
              }
            >
              {item.label}
            </NavLink>
        ...
        <div className="flex items-center gap-3">
          ...
          {isAuthenticated ? (
            <>
              <Link to={user?.role === 'admin' ? '/admin' : '/account'} className="inline-flex items-center gap-2 rounded-full border border-burgundy bg-cream px-4 py-2 text-sm font-medium text-burgundy">
                <User size={16} />
              ...
              <button
                ...
                className="inline-flex items-center rounded-full border border-slate-200 px-3 py-2 text-sm text-slate-600 hover:border-rust hover:text-rust"
              >
                Logout
              </button>
            </>
